...
class Ui_MainWindow(QtGui.QMainWindow):
    def __init__(self):#Initialization of the GUI.
        ...
        self.listOfServers = None
        self.listOfDirectories = None
        self.Onicon = QtGui.QIcon("ON.jpg")
        self.Officon = QtGui.QIcon("OFF.jpg")
        self.filechecker = None
        self.setupUi(self)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.refreshTimer.timeout.connect(self.refreshServerList)
    ...
    def start_file_checker(self):
        ...
        # self.thread.start()
        self.filechecker = FTP_watcher_Class.FTP_checker(self)
        self.filechecker.moveToThread(self.thread)
        self.filechecker.finished.connect(self.thread.quit)
        self.thread.started.connect(self.filechecker.initiator)
        self.thread.start()
    ...
    def refreshServerList(self):#function that refreshes the list of server/directories
        self.server_Directory_Tree.clear()
        if(self.filechecker !=None):
            if(self.filechecker.getDatabaseStatus() == True):
                ...
                try:
                    query = "SELECT id , serverName, host, port, userName FROM server"
                    self.databaseCursor.execute(query)
                    self.cnnct.commit()
                ...
                for counter in range(numberOfServers):
                    ...
                    self.listOfServers[counter].setExpanded(True)
                    self.server_Directory_Tree.topLevelItem(counter).setText(0, _translate("MainWindow", str(
                        serverNameAndID[counter][1]), None))
                    serverInformation = "Host: " + str(serverNameAndID[counter][2] + "\n" + "Port: " + str(serverNameAndID[counter][3]) + "\n" + "Username: " + str(serverNameAndID[counter][4]))
                    self.listOfServers[counter].setToolTip(0, serverInformation)
                    serverExistence = self.filechecker.checkServerExistence(serverNameAndID[counter][1])
                    if(serverExistence == True):
                        self.listOfServers[counter].setIcon(0,self.Onicon)
                    else:
                        self.listOfServers[counter].setIcon(0,self.Officon)


                # icon = QtGui.QIcon("FTPimage.png")
                # self.listOfServers[0].setIcon(0, icon)
    ...
    def remove_Directory(self):#function that deletes the selected directory
        itemFound = False
        if(self.listOfDirectories != None):
